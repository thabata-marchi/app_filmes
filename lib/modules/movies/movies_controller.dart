...
class MoviesController extends GetxController with MessagesMixin {
  ...
  void filterMoviesByGenre(GenreModel? genreModel) {
    ...
    print('genreSelected, ${genreSelected.value?.id}');
    print('genreModel, ${genreModel?.id}');

    print('genreSelected, ${genreSelected.value?.name}');
    print('genreModel, ${genreModel?.name}');

    if (genreModel != null) {
      print('AQUI!!!');
      var newPopularMovies = _popularMoviesOriginal.where((movie) {
        print('Movie, $movie');
        print('Movie Genre 1, ${movie.genres}');
        return movie.genres.contains(genreModel?.id);
      });

      var newTopRatedMovies = _topRatedMoviesOriginal.where((movie) {
        print('Movie Genre 2, ${movie.genres}');
        return movie.genres.contains(genreModel?.id);
      });
      popularMovies.assignAll(newPopularMovies);
      topRatedMovies.assignAll(newTopRatedMovies);
    } else {
      print('DEU RUIM, Movie');
      popularMovies.assignAll(_popularMoviesOriginal);
      topRatedMovies.assignAll(_topRatedMoviesOriginal);
    }
  }
